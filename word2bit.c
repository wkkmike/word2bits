...
void InitNet(struct vocabulary *input_vocab, struct vocabulary *output_vocab) {
   ...
   for (b = 0; b < vec_size; b++)
      for (a = 0; a < input_vocab->vocab_size; a++){
        syn0[a * vec_size + b] = (rand() / (real)RAND_MAX - 0.5) / vec_size;  //random initialize
        /*
        if(syn0[a * vec_size + b] >= 0) syn0[a * vec_size + b] = q1p;
        else syn0[a * vec_size + b] = q1n;
        */
      }
   ...
   for (b = 0; b < vec_size; b++)
      for (a = 0; a < output_vocab->vocab_size; a++)
        syn1neg[a * vec_size + b] = 0; //initialize to 0
}
     ...
     while (1) {
        ...
        for (d = 0; d < negative + 1; d++) {
           ...
           for (c = 0; c < vec_size; c++){
             if(syn0[c + l1] >= 0){
               if(syn1neg[c + l2] >= 0) f += p;
               else f += n;
             }
             else{
               if(syn1neg[c + l2] >= 0) f += n;
               else f += p;
             }
           }
           ...
           for (c = 0; c < vec_size; c++) {
            /*
            if(syn1neg[c + 12] >= 0) neu1e[c] += g * q1p;
            else neu1e[c] -= g * q1p;
            */
            neu1e[c] += g * syn1neg[c + l2];

           }// back for input vector
           for (c = 0; c < vec_size; c++){
           /*
            if(syn0[c + l1] >= 0) syn1neg[c + l2] += g * q1p;
            else syn1neg[c + l2] -= g * q1p;
           */
             syn1neg[c + l2] += g * syn0[c + l1];

           }  // back for output vector
          ...
          /*
           ...
           for (c = 0; c < vec_size; c++) f += syn0[c + l1] * syn1neg[c + l2]; // forword
           if (f > MAX_EXP) g = (label - 1) * alpha;
           else if (f < -MAX_EXP) g = (label - 0) * alpha;
           else g = (label - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
           for (c = 0; c < vec_size; c++) neu1e[c] += g * syn1neg[c + l2]; // back for input vector
           for (c = 0; c < vec_size; c++) syn1neg[c + l2] += g * syn0[c + l1]; // back for output vector
           */
        }
        for (c = 0; c < vec_size; c++) {
          syn0[c + l1] += neu1e[c];
          /*
          if(syn0[c + l1] >= 0) syn0[c + l1] = q1p;
          else syn0[c + l1] = q1n;
          syn0[c + l1] += neu1e[c];
          */
        }// back for input vector
...
void TrainModel() {
  ...
  for (a = 0; a < input_vocab->vocab_size; a++) {
    ...
    for (b = 0; b < vec_size; b++) {
      t1 = syn0[a * vec_size + b] + syn1neg[a * vec_size + b];
      if (t1 >= 0) fprintf(fo1, "1 ");
      else fprintf(fo1, "0 ");
    }
    fprintf(fo1, "\n");
  }
